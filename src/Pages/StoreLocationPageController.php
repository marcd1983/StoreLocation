...
class StoreLocationPageController extends PageController
{
    ...
    public function ContactForm(): Form
    {
        ...
        $fields = FieldList::create(
            TextField::create('Name', 'Full Name*')->setAttribute('autocomplete', 'name'),
        ...
        $actions = FieldList::create(
            FormAction::create('doContactForm', 'Send message')->addExtraClass('button primary')
        );
    ...
    public function StaffContactForm(): Form
    {
        ...
        $actions  = FieldList::create(FormAction::create('doStaffContactForm', 'Send Email')->addExtraClass('button'));
        $required = RequiredFields::create(['Name', 'Email', 'Message']);
    ...
    public function doStaffContactForm(array $data, Form $form): HTTPResponse
    {
        ...
        $recipients = $this->parseEmails($staff?->Email ?: ($this->Mailto ?? '') ?: (SiteConfig::current_site_config()->Email ?? ''));
        if (empty($recipients)) {
